...
Store hoisting 的 LB 模式 (`r1=X;Y=r1 || r2=Y;X=1 → r1=r2=1`) 在 C++11 公理模型下**允许**，但 Loom 的 operational 模型（线程内保持程序顺序）无法找到该执行。Promising Semantics 通过 promise 机制显式建模 store hoisting。

| 测试 | 场景 | C++11 | Loom | PS |
|------|------|-------|------|----|
| `test_store_hoisting_wo_dep` | 无依赖 | 允许 | 找不到 | **可达** |
| `test_store_hoisting_w_dep_oota` | 数据依赖 (OOTA) | 允许（已知缺陷） | 不可达 | **不可达** |
| `test_store_hoisting_syntactic_dep` | 语法依赖 | 允许 | 找不到 | **可达** |
| `test_store_hoisting_syntactic_dep_rw_coherence` | 语法依赖 + RW coherence | `r1=r2=1` 允许，`r3=0` | 不可达 | **不可达** |
